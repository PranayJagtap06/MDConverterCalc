...
def bckbst_cr_ind(D, R, fsw):
    ...
    """
    return np.format_float_scientific(np.divide(np.multiply(np.power(1-float(D), 2), float(R)), np.multiply(float(fsw), 2)), unique=False, precision=4, trim='-', exp_digits=1)
